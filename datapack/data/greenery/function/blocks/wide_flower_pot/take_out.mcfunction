...
execute as @e[type=interaction,tag=greenery.wide_flower_pot,distance=..5] if data entity @s interaction as @s[distance=..5] at @s run data merge entity @e[type=item_display,tag=greenery.wide_flower_pot,limit=1,sort=nearest] item:{components:{"minecraft:custom_model_data":795012}}}
